...
namespace LinkCleaner.RevitApp.Utils
{
    ...
    internal class StartPresentation : IExternalCommand, IButtonData
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            App.TryGetLinks(doc, out RevitLinkType[] AllDocumentsLinksTypes);

            //Приведение к GUID для дальнейшей работы с конфигурацией
            Guid[] AllDocumentLinksGuids = AllDocumentsLinksTypes.AsValueEnumerable().Select((link) => link.VersionGuid).ToArray();

            // Данные для вьюхи
            List<LinkModelInWPF> linksGuids = new List<LinkModelInWPF>();
